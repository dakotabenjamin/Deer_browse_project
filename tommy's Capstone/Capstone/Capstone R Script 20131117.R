...
# Look at How the NO affects the Richnesses
  ...
  hist(soildata.native$N.tot)
  hist(soildata.adventive$N.tot)
  ...
  abline(lm(soildata.adventive$rich~log(soildata.adventive$N.tot)), col="blue")    
  
  # Create Models to determine if our fit is good enough
  model_N.adventive <- lm(soildata.adventive$rich~log(soildata.adventive$N.tot)) 
  ...
  # Create models to determine if our fit is good enough
  model_N.native <- lm(soildata.native$rich~log(soildata.native$N.tot)) #lm(soildata.native$rich~log(soildata.native$N.tot))
  anova(model_N.native)
  summary(model_N.native)
  
  # N-tot didnt show anything so we will disregard it as an envronmental driver
